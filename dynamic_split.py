...
data_filename = os.path.join(cache_dir,"dynamic_features.csv.gz")
                ...
                for (i,row) in enumerate(traces_csv):
                  ...
                  if i in train_vuln_traces:
                    ...
                    for _ in range(times):
                      train_csv.writerow([row[0],row[2],0])
                  elif i in train_norm_traces:
                    times = train_norm_traces[i]
                    for _ in range(times):            
                      train_csv.writerow([row[0],row[2],1])
                  elif i in valid_vuln_traces:
                    times = valid_vuln_traces[i]
                    #assert(times == 1)
                    for _ in range(times):
                      buggy_valid_csv.writerow(([row[0],row[2],0])) 
                  elif i in valid_norm_traces:
                    times = valid_norm_traces[i]
                    #assert(times == 1)
                    for _ in range(times):
                      robust_valid_csv.writerow(([row[0],row[2],1]))
                  elif i in test_vuln_traces:
                    times = test_vuln_traces[i]
                    assert(times == 1)
                    for _ in range(times):
                      buggy_test_csv.writerow(([row[0],row[2],0])) 
                  elif i in test_norm_traces:
                    times = test_norm_traces[i]
                    assert(times == 1)
                    for _ in range(times):
                      robust_test_csv.writerow(([row[0],row[2],1]))
